...
fake = 'Pinky Self'
fx = '➤'
lolhuman = 'e966d3aac613b61dabd35cc6'
numbernye = '0'
ban = []
waktu = '-'
alasan = '-'
...
async function starts() {
	...
	fxsx.on('qr', () => {
		console.log(color('[','red'), color('!','yellow'), color(']','red'), color(' Scan Qr Nya Kak', 'green'))
	})

	fs.existsSync('./pinky.json') && fxsx.loadAuthInfo('./pinky.json')
	fxsx.on('connecting', () => {
		start('2', 'Otw Pinky Meluncur...')
	})
	fxsx.on('open', () => {
		success('2', 'Nyampe Punten Goput')
	})
	await fxsx.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./pinky.json', JSON.stringify(fxsx.base64EncodedAuthInfo(), null, '\t'))
...
fxsx.on('group-participants-update', async (anu) => {
       ...
       if (anu.action == 'add') {
		...
		fxsx.sendMessage(mdata.id, buffer, MessageType.image, {quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: `Welcome @${num.split('@')[0]}\n${tampilTanggal}`, orderTitle: `Welcome @${num.split('@')[0]}\n${tampilTanggal}`, thumbnail: fs.readFileSync('fake.jpeg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: {"mentionedJid": [num]}, caption: teks})
		}
            if (anu.action == 'remove') {
                ...
                fxsx.sendMessage(mdata.id, buffer, MessageType.image, {quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: `Keluar @${num.split('@')[0]}\n${tampilTanggal}`, orderTitle: `Keluar @${num.split('@')[0]}\n${tampilTanggal}`, thumbnail: fs.readFileSync('fake.jpeg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: {"mentionedJid": [num]}, caption: teks})
            }
		} catch (e) {
		console.log(e)
	 }
	...
	fxsx.on('chat-update', async (mek) => {
		...
		const fimg = await getBuffer(me.imgUrl)
		...
		const sender = mek.key.fromMe ? fxsx.user.jid : isGroup ? mek.participant : mek.key.remoteJid
        ...
        const ftroli ={"key": {   "fromMe": false,"participant":"0@s.whatsapp.net",   "remoteJid": "0@s.whatsapp.net"  }, "message": {orderMessage: {itemCount: 1,status: 200, thumbnail: fs.readFileSync('thumb.jpeg'), surface: 200, message: `${fake}\nHai ${pushname}`, orderTitle: 'NuyFaa', sellerJid: '0@s.whatsapp.net'}}, contextInfo: { forwardingScore: 508, isForwarded: true}}
		...
		mess = {
			...
			userB: 'Hai Kak Daftar Dulu...',
			benned: 'Maaf Kamu Sudah Terbenned...',
			ownerB: 'Maaf Khusus Owner Ku...',
			error: {
				stick: 'Bukan Sticker...',
				Iv: 'Link Eror...'
			},
...
//FUNCTION
            ...
            fxsx.sendMessage(mek.key.remoteJid,`@${owner} Sedang Offline!\n\n*Alasan :* ${alasan}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik lalu\n\nSilahkan Hubungi Lagi Nanti`, MessageType.text,{contextInfo:{ mentionedJid: [`${owner}@s.whatsapp.net`],'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': "0@s.whatsapp.net", 'remoteJid': 'status@broadcast', 'quotedMessage': {"imageMessage": {"caption": "*OFFLINE*", 'jpegThumbnail': fs.readFileSync('thumb.jpeg')}}}})
            }
            }   
        if (mek.key.remoteJid.endsWith('@g.us') && offline) {
        if (!mek.key.fromMe){
...
switch (command) {
    ...
    case 'menu':
    case 'help':
        if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
        if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
    	var menu = `Selamat ${tampilUcapan}
Kak : ${pushname}
Terdaftar : √
...
${fx} ${prefix}darkjokes
${fx} ${prefix}asupan
${fx} ${prefix}asupansantuy
${fx} ${prefix}asupanbocil
${fx} ${prefix}asupanukhty
${fx} ${prefix}asupanrikagusriani
${fx} ${prefix}asupanghea
${fx} ${prefix}quotes
╰──────────
...
${fx} *PINKY•SELF*`
        	fxsx.sendMessage(from, menu, MessageType.text, {quoted: ftroli})
           	break
    case 'owner':
    case 'ow':
            if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
    ...
    case 'kontag':
            if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
            if (!isGroupAdmins) return fxsx.sendMessage(from, `Maaf ${pushname} Command Khusus Admin Grup`,MessageType.text, { quoted: ftroli})
            if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
            ...
            break
    case 'kalender':
            fxsx.sendMessage(from, `${tampilTanggal}`,MessageType.text, { quoted: ftroli})
            break
    case 'sticktag':
            if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
            if (!isGroupAdmins) return fxsx.sendMessage(from, `Maaf ${pushname} Command Khusus Admin Grup`,MessageType.text, { quoted: ftroli})
            ...
            break
    case 'pinky':
           if (!isOwner) return fxsx.sendMessage(from, 'Khusus Owner Pinky',MessageType.text, { quoted: ftroli})
    ...
    case 'adds':
           if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
           if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
		   if (!isQuotedSticker) return fxsx.sendMessage(from, 'Reply stiker nya',MessageType.text, { quoted: ftroli})
		   svst = body.slice(12)
	       if (!svst) return fxsx.sendMessage(from, 'Nama sticker nya apa?',MessageType.text, { quoted: ftroli})
		   ...
		   fxsx.sendMessage(from, `Sukses Menambahkan Sticker\nCek dengan cara ${prefix}slist`,MessageType.text, { quoted: ftroli})
		   break
    case 'slist':
    case 'lists':
		   if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
    ...
    case 'gets':
	       ...
	       fxsx.sendMessage(from, result, sticker, {quoted: ftroli})
		   break
    case 'addvn':
           if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
           if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
		   if (!isQuotedAudio) return fxsx.sendMessage(from, 'Reply Audio Nya',MessageType.text, { quoted: ftroli})
		   svst = body.slice(7)
		   if (!svst) return fxsx.sendMessage(from, 'Nama audionya?',MessageType.text, { quoted: ftroli})
		   boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
		   delb = await fxsx.downloadMediaMessage(boij)
		   audionye.push(`${svst}`)
	       fs.writeFileSync(`./storage/audio/${svst}.mp3`, delb)
		   fs.writeFileSync('./storage/audio.json', JSON.stringify(audionye))
		   fxsx.sendMessage(from, `Sukses Menambahkan Audio\nCek dengan cara ${prefix}vnlist`,MessageType.text, { quoted: ftroli})
		   break
    case 'vnlist':
	case 'listvn':
           if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
    ...
    case 'addvid':
	       if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
           if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
	       if (!isQuotedVideo) return fxsx.sendMessage(from, 'Reply videonya!',MessageType.text, { quoted: ftroli})
		   svst = body.slice(10)
	       if (!svst) return fxsx.sendMessage(from, 'Nama videonya apa?',MessageType.text, { quoted: ftroli})
		   boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
	       delb = await fxsx.downloadMediaMessage(boij)
		   videonye.push(`${svst}`)
		   fs.writeFileSync(`./storage/video/${svst}.mp4`, delb)
		   fs.writeFileSync('./storage/video.json', JSON.stringify(videonye))
		   fxsx.sendMessage(from, `Sukses Menambahkan Video\nCek dengan cara ${prefix}vidlist`,MessageType.text, { quoted: ftroli})
		   break
    case 'vidlist':
    case 'listvid':
	        if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
		    ...
		    fxsx.sendMessage(from, teks.trim(), extendedText, { quoted: ftroli, contextInfo: { "mentionedJid": videonye } })
		    break
    case 'getvid':
    case 'gvid':
            if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
		    ...
		    fxsx.sendMessage(from, buffer, video, { mimetype: 'video/mp4', quoted: ftroli})
	        break
    case 'fitnah':
            if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
            if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
    ...
    case 'quotebucin':
             ...
             fxsx.sendMessage(from, '*'+hasil+'*', MessageType.text, {quoted: ftroli})
             break
    case 'darkjokes':
		     if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
			 if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
			 fxsx.updatePresence(from, Presence.composing)
			 data = fs.readFileSync('./pinkyca/drak.js');
             jsonData = JSON.parse(data);
             randIndex = Math.floor(Math.random() * jsonData.length);
             randKey = jsonData[randIndex];
             darkjokes = await getBuffer(randKey.result)
             fxsx.sendMessage(from, darkjokes, image, {quoted: ftroli, caption: 'Gelap:V'})
			 break  
	case 'asupan':
	         if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
             if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
             fxsx.updatePresence(from, Presence.composing)
             data = fs.readFileSync('./pinkyca/asupan.js')
             jsonData = JSON.parse(data)
             randIndex = Math.floor(Math.random() * jsonData.length);
             randKey = jsonData[randIndex];
             asupan = await getBuffer(randKey.result)
             fxsx.sendMessage(from, asupan, video, { mimetype: 'video/mp4', quoted: ftroli, caption: 'Sekedar Untuk Gabut' })
             break
    case 'asupansantuy':
	case 'asupanbocil':
	case 'asupanukhty':
	...
	case 'asupanghea':
			 ...
			 fxsx.sendMessage(from, buffer, video, { mimetype: 'video/mp4', quoted: ftroli, caption: 'Sekedar Untuk Gabut' })
			 break
    case 'quotes':
			 if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
			 if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
			 data = fs.readFileSync('./pinkyca/quotes.js');
			 jsonData = JSON.parse(data);
			 randIndex = Math.floor(Math.random() * jsonData.length);
			 randKey = jsonData[randIndex];
			 randQuote = 'Author: *'+randKey.author+'*\n\n*'+randKey.quotes+'*'
			 fxsx.sendMessage(from, randQuote, MessageType.text, {quoted: ftroli})
			 break 
    case 'pinkyca':
            if (!isOwner) return fxsx.sendMessage(from, 'Khusus Owner Pinky',MessageType.text, { quoted: ftroli})
		    if (args.length < 1) return fakestatus('Jumlahnya?')
		    for (let i = 0; i < args[0]; i++) {
            await fxsx.toggleDisappearingMessages(from, 0)
            }
            fakestatus(from, 'Sukses... '+args.join(' '))
			break
    case 'settarget':
    ...
    case 'colong':
		    ...
		    media = await fxsx.downloadAndSaveMediaMessage(encmedia)
            anu = args.join(' ').split('|')
            satu = anu[0] !== '' ? anu[0] : `Pinky`
            dua = typeof anu[1] !== 'undefined' ? anu[1] : `Caa`
    ...
    case 'swm':
            ...
            .on('error', (e) => {
            console.log(e)
            fxsx.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: ftroli })
            fs.unlinkSync(media)
            })
            .on('end', () => {
            _out = getRandom('.webp')
    ...
    case 'banlist':
			...
			fxsx.sendMessage(from, ben.trim(), extendedText, {quoted: ftroli, contextInfo: {"mentionedJid": ban}})
			break
    case 'ban':
			fxsx.updatePresence(from, Presence.composing) 
			if (!isOwner) return fxsx.sendMessage(from, 'Khusus Owner Pinky',MessageType.text, { quoted: ftroli})
			...
			mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
			ban = mentioned
			fxsx.sendMessage(from, `Berhasil Banned : ${ban}`,MessageType.text, { quoted: ftroli})
			break
    case 'unban':
		    if (!isOwner) return fxsx.sendMessage(from, 'Khusus Owner Pinky',MessageType.text, { quoted: ftroli})
			bnnd = body.slice(8)
			ban.splice(`${bnnd}@s.whatsapp.net`, 1)
			fxsx.sendMessage(from, `Nomor wa.me/${bnnd} telah di unban!`,MessageType.text, { quoted: ftroli})
			break
    case 'affect':
    case 'beautiful':
    case 'facepalm':
    ...
    case 'wanted':
            ...
            buffer = await getBuffer(`https://api.lolhuman.xyz/api/creator1/${command}?apikey=${lolhuman}&img=${anu.display_url}`)
            fxsx.sendMessage(from, buffer, image, {quoted: ftroli})
            } else {
            fxsx.sendMessage(from, 'Reply Imagenya!!',MessageType.text, { quoted: ftroli})
            }
            break
 	case 'hidetag':
 	        if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
	...
	case 'adminslist':
			if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
			if (!isGroup) return fxsx.sendMessage(from, 'Khusus Group Kak',MessageType.text, { quoted: ftroli})
			if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
	...
	case 'cekchat':
			...
			fxsx.sendMessage(from, teks, MessageType.text, selepbot)
		    break
    case 'grouplist':
			if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
			if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
	...
	case 'ytsearch':
        	...
        	ytresult += `${fx} Title: ` + video.title + '\n'
            ytresult += `${fx} Link: ` + video.url + '\n'
            ytresult += `${fx} Durasi: ` + video.timestamp + '\n'
            ytresult += `${fx} Upload: ` + video.ago + '\n________________________\n\n'
    		});
    		ytresult += `${fx} *SELF-BOT*`
    ...
    case 'setlol':
			...
			if (!q) return fakegroup(mess.wrongFormat)
			lolhuman = q
			fakegroup(`Succes Mengganti Conversation Apikey lolhuman : ${q}`)
			break
	case 'setfakeimg':
	        if (!isOwner) return fxsx.sendMessage(from, 'Khusus Owner Pinky',MessageType.text, { quoted: ftroli})
			...
			delb = await fxsx.downloadMediaMessage(boij)
			fs.writeFileSync(`fake.jpeg`, delb)
			fakestatus('Sukses')
        	} else {
	...
	case 'setthumb':
	        ...
	        if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedSticker) && args.length == 0) {
          	boij = isQuotedImage || isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
			delb = await fxsx.downloadMediaMessage(boij)
			fs.writeFileSync(`thumb.jpeg`, delb)
			fakestatus('Sukses')
        	} else {
            reply(`Kirim gambar dengan caption ${prefix}sethumb`)
          	}
	...
	case 'setppbot':
			...
			fxsx.sendMessage(from, 'Makasih profile barunya owner😉',MessageType.text, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}})
			break 
	case 'ytmp4':
	        if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
			if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
    ...
    case 'ig':
        if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
        if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
        if (!isUrl(args[0]) && !args[0].includes('https://www.instagram.com/')) return reply(mess.Iv)
        if (!q) return fakegroup('Linknya?')
        reply(mess.wait)
	    igdl(args[0])
	    .then(async(result) => {
    ...
    case 'fb':
            ...
            if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
            if (!q) return reply('Linknya?')
            if (!isUrl(args[0]) && !args[0].includes('https://www.facebook.com/')) return reply(mess.Iv)
            reply(mess.wait)
            te = args.join(' ')
...
${descOwner ? `*Desc diubah oleh* : @${descOwner.split('@')[0]}` : '*Desc diubah oleh* : -'}\n*Tanggal* : ${descTime ? `${formatDate(descTime * 1000)}` : '-'}\n\n*Kontak yang tersimpan*\n`
    ...
    case 'totaluser':
	         ...
	         fxsx.sendMessage(from, teks.trim(), extendedText, {quoted: ftroli, contextInfo: {"mentionedJid": user}})
	         break
    case 'daftar':
			 fxsx.updatePresence(from, Presence.composing)
			 if (isUser) return fxsx.sendMessage(from, `${pushname} Kamu Sudah Terdaftar`,MessageType.text, { quoted: ftroli})
			 if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
			 user.push(sender)
			 ...
			 captionnya = `╭─${fx}「 *Berhasil Terdaftar* m」\n│\n│${fx} Dengan Sn\n│${fx} TM08GK8PPHBSJDH10J\n│\n│${fx} Pada ${tampilTanggal} ${tampilWaktu}\n│${fx} Nama : ${pushname}\n│${fx} Nomer : wa.me/${sender.split('@')[0]}\n│${fx} Total User : ${user.length} Orang\n│\n│${fx} Menggunakan Pinky Bot\n│${fx} Ketik : #menu\n│\n│${fx} Note\n│${fx} No Spam Bot!!\n╰─────────────────────`
             buffer = await getBuffer(ppimg)
			 fxsx.sendMessage(from, buffer, image, {quoted: {"key": {   "fromMe": false,"participant":"0@s.whatsapp.net",   "remoteJid": "0@s.whatsapp.net"  }, "message": {orderMessage: {itemCount: 1,status: 200, thumbnail: fs.readFileSync('thumb.jpeg'), surface: 200, message: `${fake}\nHai ${pushname}`, orderTitle: 'NuyFaa', sellerJid: '0@s.whatsapp.net'}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: captionnya}})
			 break
    case 'welcome':
			 if (!isGroup) return fxsx.sendMessage(from, 'Khusus Group Kak',MessageType.text, { quoted: ftroli})
			 if (!isGroupAdmins) return fxsx.sendMessage(from, `Maaf ${pushname} Command Khusus Admin Grup`,MessageType.text, { quoted: ftroli})
			 if (args.length < 1) return fxsx.sendMessage(from, 'Mengaktifkan Ketik On, Menonaktif Ketik Off',MessageType.text, { quoted: ftroli})
			 if (args[0] === 'on') {
			 	if (isWelkom) return fxsx.sendMessage(from, '*Fitur welcome sudah aktif sebelum nya',MessageType.text, { quoted: ftroli})
			 	welkom.push(from)
			 	fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
			 	fxsx.sendMessage(from, '❬ SUCCSESS ❭ mengaktifkan fitur welcome di group ini',MessageType.text, {quoted: ftroli})
			 } else if (args[0] === 'off') {
			 	welkom.splice(from, 1)
			 	fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
			 	fxsx.sendMessage(from, '❬ SUCCSESS ❭ menonaktifkan fitur welcome di group ini',MessageType.text, {quoted: ftroli})
			 } else {
			 	fxsx.sendMessage(from, 'on untuk menyalakan, off untuk mematikan',MessageType.text, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
			 }
			 break

       default:
       if (body.startsWith(`${prefix}${command}`)) {
       const loli = fs.readFileSync('./mp3/tidak.mp3')
       fxsx.sendMessage(from, loli, MessageType.audio, { quoted: mek, mimetype: 'audio/mp4', ptt: true })
       }
       }
    if (isGroup && !isCmd && budy != undefined) {
						...
						console.log(color('[PINKY]','aqua'), 'SELF-MODE', color(sender.split('@')[0]))
					}
		} catch (e) {
